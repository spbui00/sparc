...
def main():
    ...
    raw = data_handler.load_npz_data('../../data/simulated_data_2x64_1000.npz')
    data_obj = SimulatedData(
        raw_data=raw['raw_data'],
        sampling_rate=raw['sampling_rate'],
        ground_truth=raw['ground_truth'],
        artifacts=raw['artifacts'],
        artifact_markers=ArtifactTriggers(starts=raw['artifact_markers']),
        firing_rate=raw['firing_rate'],
        spike_train=raw['spike_train'],
        lfp=raw['lfp'],
        stim_params=None,
        snr=raw['snr'],
    )
    ...
    print("Simulated LFP metrics (trial 0) FOR GROUND TRUTH:")
    ...
    plt.title('Simulated LFP PSD (mean over trials)')
    plt.xlabel('Frequency (Hz)')
    plt.ylabel('Power')
    plt.grid(True, linestyle='--', alpha=0.5)
    outdir = os.path.join(os.path.dirname(__file__), '..', '..', 'results')
    os.makedirs(outdir, exist_ok=True)
    outpath = os.path.join(outdir, 'simulated_lfp_psd.png')
    plt.tight_layout()
