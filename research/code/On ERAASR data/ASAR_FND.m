...
fs_data = 30000; % sampling frequency Hz
    ...
    for ch = 1:total_channels
        ...
        for n =1:num_timesteps
                ...
                if(abs(Dout(n)-avg)>=a*std)
                    ...
                    Dout_clean(trial_num,n,ch)=Dout2(n)- u*w1;
        end
...
time_ms = (0:size(data_in, 2)-1) / (fs_data / 1000); 

figure();
plot(time_ms, data_in(selected_trial_number, :, selected_channel_number));
hold on;
plot(time_ms, Dout_clean(selected_trial_number, :, selected_channel_number));
hold off;
